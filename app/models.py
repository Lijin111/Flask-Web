...
from flask import current_app,url_for,request, request
import hashlib
import bleach
...
class Role(db.Model):
    ...
    def __repr__(self):
        return '<Role %r default %r>' % (self.name, self.default)
...
class User(UserMixin, db.Model):
    ...
    @staticmethod
    def DeleteNameIsNone():
        for u in User.query.filter_by(username=None):
            db.session.delete(u)
        db.session.commit()
    ...
    def __repr__(self):
        return '<User %r, Role %r>' % (self.username, self.role_id)
